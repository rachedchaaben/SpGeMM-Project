...
void spgemm_insert_row2(std::unordered_set<IndexType>& cols,
                        ...
                        size_type row, IndexType start_b, IndexType end_b)
{
    ...
    for (size_type a_nz = a_row_ptrs[row];
        ...
        cols.insert(b_col_idxs + col_start, b_col_idxs + col_end);
    }
}
    ...
    std::map<std::pair<int, int>, double>& a)
{
    for (const auto& entry : a) {
        ...
        result.push_back(std::tuple<int, int, double>(
            key_pair.first, key_pair.second, entry.second));
    }
}
    ...
    for (auto row_idx = a_offsets.begin() + 1; row_idx < a_offsets.end();
        ...
        for (auto col_idx = b_offsets.begin() + 1; col_idx < b_offsets.end();
            ...
            if (overlap<double, int>(a, b_T.get(), *(row_idx - 1), *row_idx,
                                     *(col_idx - 1), *col_idx)) {
                // std::cout<< *row_idx-*(row_idx - 1)<<' '<< *col_idx-*(col_idx
                // - 1)<<'\n';
                /*unit_spgemm3<double, int>(a_T.get(), b, result, *(row_idx -
    ...
    std::vector<std ::tuple<IndexType, IndexType, ValueType>>& result)
{
    ...
    if (h_recursive_splits == 0 || num_cols < 2) {
        ...
        h_recursive_spgemm(a, b, a_T, b_T, h_recursive_splits - 1, start_a,
                           end_a, start_b, b_idx, result);
        h_recursive_spgemm(a, b, a_T, b_T, h_recursive_splits - 1, start_a,
                           end_a, b_idx, end_b, result);
    }
}
...
TEST(example, v_recursive_spgemm)
{
    ...
    A->apply(B.get(), C.get());
    std::vector<std ::tuple<int, int, double>> result;
    v_recursive_spgemm<double, int>(A.get(), B.get(), a_T.get(), b_T.get(), 2,
                                    2, 0, A->get_size()[0], 0, B->get_size()[1],
                                    result);
    std::sort(result.begin(), result.end());
    auto entry = result.begin();
...
{
    for (auto _ : state){
    bool test= overlap<double,int>(A.get(), b_T.get(), 0,A->get_size()[0],
                                     0, B->get_size()[1]);
    }
}
